...
begin
  ...
  select pg_get_functiondef('public.coordinator_update_shift(uuid,text,timestamp with time zone,timestamp with time zone,jsonb,public.shift_status,boolean,text,text,public.event_format,text)'::regprocedure) into v_update_definition;
