...
interface UpdatesSignupProps {
  onSubscribe?: (values: UpdatesSignupValues) => void;
}
    ...
    <main className="min-h-full bg-[#F5F1E8] px-6 py-14 text-[#1F2421] sm:px-10 lg:px-16">
      <div className="mx-auto grid max-w-5xl gap-12 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <form className="bg-white/55 p-6 sm:p-8" onSubmit={submit}>
          <label className="block"><span className="text-sm font-semibold">Name <span aria-hidden="true">*</span></span><input autoComplete="name" className="mt-2 block w-full rounded-sm border border-[#6F8580]/55 bg-white px-3 py-2.5 focus:border-[#3F5F5B] focus:outline-none focus:ring-2 focus:ring-[#3F5F5B]/25" onChange={(event) => setValues({ ...values, name: event.target.value })} required value={values.name} /></label>
          ...
          <button className="mt-7 rounded-sm bg-[#3F5F5B] px-5 py-3 text-sm font-semibold text-[#F5F1E8] transition hover:bg-[#2e4945] focus:outline-none focus:ring-2 focus:ring-[#3F5F5B] focus:ring-offset-2" type="submit">Subscribe to updates</button>
        </form>
      </div>
    </main>
  );
